...
const VARIANTS = {
  primary: 'bg-navy text-paper hover:bg-navy-dark shadow-soft border border-navy',
  accent:  'bg-maroon text-paper hover:bg-maroon-dark shadow-gold border border-maroon',
  ghost:   'bg-transparent text-navy border border-line hover:bg-navy/8 hover:border-line-strong',
  surface: 'bg-paper text-ink border border-line hover:bg-cream-dark',
  danger:  'bg-maroon/10 text-maroon border border-maroon/25 hover:bg-maroon/18',
}
...
export function Spinner({ dark, light }) {
  const cls = light ? 'spinner-light' : dark ? 'spinner-dark' : 'spinner'
...
export function Modal({ title, onClose, children, maxWidth = 'max-w-md' }) {
  ...
  return (
    <div
      ...
      ref={dialogRef}
      className="fixed inset-0 z-[1000] flex items-center justify-center p-5 bg-navy/30 backdrop-blur-sm animate-fadeIn"
      onClick={onClose}
    >
      <div
      ...
      >
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-line">
          <h2 id={titleId} className="text-[19px] font-bold text-navy tracking-tight font-display">{title}</h2>
          <button
            onClick={onClose}
            aria-label="Fechar"
...
export function Input({ id, ...props }) {
  return (
    <input
      id={id}
      className="w-full bg-cream border-[1.5px] border-line rounded-[10px] text-ink text-sm
