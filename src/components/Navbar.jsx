...
export default function Navbar({ user, onLogin, onLogout }) {
  ...
  function handleInstall() {
    if (pwa.isIos) { setShowIosHint((v) => !v); return; }
  ...
  return (
    <>
      ...
      <nav
        ...
        px-4 sm:px-6 lg:px-10 flex items-center justify-between shadow-soft"
      >
        {/* Logo */}
        <button
          onClick={() => go("/")}
          ...
          {NAV.map(({ label, path }) => (
            ...
            >
              ...
              {isActive(path) && (
                <span
                  aria-hidden="true"
                  className="absolute -bottom-0.5 left-0 right-0 h-0.5 bg-maroon rounded-full"
                />
              )}
            </button>
          ))}
          ...
          {user ? (
            <button
              ...
              className={`flex flex-col items-center gap-0.5 p-2 rounded-lg transition-colors
                ${isActive("/dashboard") ? "text-navy" : "text-ink-soft"}`}
            >
        ...
        {pwa.show && (
          <div className="relative">
            ...
            {pwa.isIos && showIosHint && (
              <>
                <div className="fixed inset-0 z-[800]" onClick={() => setShowIosHint(false)} aria-hidden="true" />
                <div
                  role="tooltip"
...
function HomeIcon({ size = 24, active }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor"
      strokeWidth={active ? 2.2 : 1.8} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" focusable="false">
